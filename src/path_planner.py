...
class Graph:
    ...
    def get_closest_node(self, xy):
        ...
        for i in range(len(self.nodes_)):
            ...
            distance = math.sqrt((xy[0] - self.nodes_[i].x)**2 + (xy[1] - self.nodes_[i].y)**2)

        # If the distance is less than the best distance, update the best distance and index
...
class GraphSearch:
    ...
    def search(self, start_idx, goal_idx):
        ...
        while len(unvisited_set) > 0:
            ...
            node_idx = unvisited_set.pop(unvisited_idx)
    ...
    def find_connected_nodes(self, start_idx):
        ...
        while stack:
            ...
            neighbors = self.graph_.nodes_[node_idx].neighbours
            for neighbor in neighbors:
                neighbor_idx = self.graph_.nodes_.index(neighbor)
                if neighbor_idx not in visited_set:
...
class PathSmoother():
    ...
    def smooth_path(self, path_nodes):
        ...
        # Convert path into a geometry_msgs.Point[]
        path = []
        ...
        while delta > tolerance:
            ...
            for i in range(len(path_smooth)-2):
                ...
                start = path_smooth[i+1]
                start_minus1 = path_smooth[i]
                start_plus1 = path_smooth[i+2]
                path_i = path[i+1]

                # compute new path point using the formula
                star_newX = start.x - (alpha + 2*beta)*start.x + alpha*path_i.x + beta*start_minus1.x + beta*start_plus1.x
                start_newY = start.y - (alpha + 2*beta)*start.y + alpha*path_i.y + beta*start_minus1.y + beta*start_plus1.y
                #s_i_new = [s_i_new_x,s_i_new_y]
                start_newI = Point()
                start_newI.x = star_newX
                start_newI.y = start_newY
                
                # store in new_path_smooth list
                new_path_smooth[i+1] = start_newI
